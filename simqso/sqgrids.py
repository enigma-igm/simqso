...
class PowerLawSampler(CdfSampler):
    ...
    def ppf(self, y):
        if np.any(y < 0) or np.any(y > 1):
            raise ValueError
        x1, x2, a = self.low, self.high, self.a
        return np.power((x2 ** (a + 1) - x1 ** (a + 1)) * y + x1 ** (a + 1),
                        (a + 1) ** -1)
...
class BrokenPowerLawContinuumVar(ContinuumVar, MultiDimVar):
    ...
    def total_flux(self, slopes, fluxNorm, z, minwave=12.4, maxwave=1.3e4):
        ...
        ftot = np.sum(
            breakwvs[:-1] * f0 * (np.power(waveratio, alpha_lams + 1) - 1)
            / (alpha_lams + 1))
        return ftot
        ...
        for i, v in enumerate(range(hdr['NSIMVAR'])):
            ...
            try:
                ...
                if name in self.data.colnames:
                    ...
                    # quick hack, JTS Nov 20 2018
                    initargs.append(smplr)
                    # self.fixedVars.append(name)
...
def generateBEffEmissionLines(M1450, **kwargs):
    trendFn = kwargs.pop('EmissionLineTrendFilename', 'emlinetrends_v7')
    ...
    #
    useLines = ~np.in1d(np.array(lineCatalog['name']), excludeLines)
...
class VW01FeTemplateGrid(object):
    ...
    def _loadVW01Fe(self, wave):
        ...
        for t in templnames:
            ...
            w1, w2 = np.searchsorted(wave,
                                     [tspec['wave'][0], tspec['wave'][-1]])
    ...
    def _restFrameFeTemplate(self, FWHM_kms, feScalings):
        ...
        gkern = np.exp(-x ** 2 / (2 * sigma_conv ** 2)) / (
                np.sqrt(2 * np.pi) * sigma_conv)
